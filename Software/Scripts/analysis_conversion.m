...
%     main_print(strcat("Slush Hydrogen Pathway Conversion Efficency: ",string(100*ConversionCosts.SH2_H2.Efficency)," %%"),'a');
    ...
    for n = 1:10
        ConversionCosts.volume(n) = Conversion.CAPACITY(n) * Conversion.Lifespan * 1000;
        ConversionCosts.totalcost(n) = Conversion.CAPEX(n) + Conversion.OPEX(n) * Conversion.Lifespan;
        ConversionCosts.energy(n) = Conversion.ENERGY(n) * ConversionCosts.volume(n);
        ConversionCosts.energycost(n) = ConversionCosts.energy(n) .* 0.3;
        ConversionCosts.unitcost(n) = (ConversionCosts.totalcost(n) + ConversionCosts.energycost(n))/ConversionCosts.volume(n);
        ConversionCosts.ConvUnitCost(n) = Conversion.Proportion(n) * ConversionCosts.unitcost(n);
        ConversionCosts.ReConvUnitCost(n) = (1 - Conversion.Proportion(n)) * ConversionCosts.unitcost(n);
    end
